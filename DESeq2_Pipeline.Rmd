...
# read raw data
path_to_counts <-"~/ADD3_computable_paper/data/salmon_merged_counts.csv"
raw <- read.csv(file=path_to_counts, header=T, row.names=1, stringsAsFactors=F)
...
# remove samples that do not reach the threshold
gnames <- rownames(raw)
...
```{r, echo=TRUE}
tab_Bhaduri <- read.xlsx("~/ADD3_computable_paper/data/Bhaduri_GBM.xlsx")
tab_Kalebic <- read.xlsx("~/ADD3_computable_paper/data/Morpho genes in bRG.xlsx",colNames = F)
...
CMP_ANN <- read.csv("~/ADD3_computable_paper/data/model_list_depmap.csv")
GBM_CMP <- CMP_ANN %>% 
